...
# Get features and labels from csv file
    ...
    #Parameters
    num_classes = 6 # The 6 classes of servants
...
# Organize data to be used for training
    ...
    # Create lists for Serving Group Model
        ...
        serve_group_features = tf.placeholder(tf.float32, shape=[None, num_features])
        # For random forest, labels must be integers (the class id)
        serve_group_classes = tf.placeholder(tf.int32, shape=[None])
...
# Train and test Models
    ...
    train_op = forest_graph.training_graph(serve_group_features, serve_group_classes)
    loss_op = forest_graph.training_loss(serve_group_features, serve_group_classes)

    regressor = tf.contrib.learn.TensorForestEstimator(hparams)
    regressor.fit(x=serve_group_features, y=serve_group_classes, num_steps)
