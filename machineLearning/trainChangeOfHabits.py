...
# Get features and labels from csv file
    ...
    #Parameters
    num_classes = 6 # The 6 classes of habits to improve
    num_features = 25 # Amount of questions we are asking

# Organize data to be used for training

    # Create lists for Change of Habits Model
        # Input and Target data
        coh_features = tf.placeholder(tf.float32, shape=[None, num_features])
        # For random forest, labels must be integers (the class id)
        coh_classes = tf.placeholder(tf.int32, shape=[None])
...
# Train and test Models
    ...
    train_op = forest_graph.training_graph(coh_features, coh_classes)
    loss_op = forest_graph.training_loss(coh_features, coh_classes)

    regressor = tf.contrib.learn.TensorForestEstimator(hparams)
    regressor.fit(x=coh_features, y=coh_classes, num_steps)
