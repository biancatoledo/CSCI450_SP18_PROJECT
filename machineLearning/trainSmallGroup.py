...
# Get features and labels from csv file
    ...
    #Parameters
    num_classes = 4 # The 4 classes of small groups
    num_features = 25 # Amount of questions we are asking

# Organize data to be used for training


    # Create lists for Small Group Model
        # Input and Target data
        small_group_features = tf.placeholder(tf.float32, shape=[None, num_features])
        # For random forest, labels must be integers (the class id)
        small_group_classes = tf.placeholder(tf.int32, shape=[None])
...
# Train and test Models
    ...
    train_op = forest_graph.training_graph(small_group_features, small_group_classes)
    loss_op = forest_graph.training_loss(small_group_features, small_group_classes)

    regressor = tf.contrib.learn.TensorForestEstimator(hparams)
    regressor.fit(x=small_group_features, y=small_group_classes, num_steps)
